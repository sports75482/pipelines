...
def input_dictionary_to_parameter(input_dict: Dict[str, Any]) -> str:
...
def get_skip_evaluation_pipeline_and_parameters(
    ...
    stage_1_tuner_worker_pool_specs_override: str = '',
    cv_trainer_worker_pool_specs_override: str = '',
    ...
    encryption_spec_key_name: str = '') -> Tuple[str, Dict[str, str]]:
  ...
  Args:
    ...
    transform_dataflow_disk_size_gb: Dataflow worker's disk size in GB for
      transform component.
    dataflow_subnetwork: Dataflow's subnetwork name, when empty the default
